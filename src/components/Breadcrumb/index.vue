<template>
	<a-breadcrumb class="breadcrumb">
		<a-breadcrumb-item v-for="(item, index) in breadcrumbList" :key="index">
			<span v-if="item.redirect === 'noRedirect' || index === breadcrumbList.length - 1" class="breadcrumb-item">{{ item.meta.title }}</span>
			<a v-else class="breadcrumb-item" @click="handleLink(item)">{{ item.meta.title }}</a>
		</a-breadcrumb-item>
	</a-breadcrumb>
</template>
