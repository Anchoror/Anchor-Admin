<template>
	<div v-if="!item.meta?.hidden">
		<template
			v-if="hasOneShowingChild(item.children, item) && (!onlyOneChild?.children || onlyOneChild?.meta?.noShowingChildren) && !item?.meta?.alwaysShow"
		>
 ...
 */
function resolvePath(path: string, routeQuery?: any): string | { path: string; query: AnyObject } {
	if (isExternal(path)) {
		return path;
	}
	if (routeQuery) {
